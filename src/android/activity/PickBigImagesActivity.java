...
public class PickBigImagesActivity extends Activity implements ViewPager.OnPageChangeListener, View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(fakeR.getId("layout", "activity_pick_big_images"));
        initFindView();
        initData();
    }

    protected void initFindView() {
        viewPager = (ViewPager) findViewById(fakeR.getId("id", "vp_content"));
        tv_choose_pic = (TextView) findViewById(fakeR.getId("id", "tv_choose_pic"));
        iv_choose_state = (ImageView) findViewById(fakeR.getId("id", "iv_choose_state"));
        ...
        if(last_pics < total_pics) {
            ...
            btn_choose_finish.setText(String.format(getString(fakeR.getId("string", "choose_pic_finish_with_num")), total_pics - last_pics, total_pics));
        }
    }
    ...
    protected void initData() {
        ...
        if (getChooseStateFromList(currentPic)){
            iv_choose_state.setBackgroundResource(fakeR.getId("drawable", "image_choose"));
        }else{
            iv_choose_state.setBackgroundResource(fakeR.getId("drawable", "image_not_chose"));
        }
    ...
    public void onPageSelected(int position) {
        ...
        currentPic = position;
        ((TextView)findViewById(fakeR.getId("id", "tv_title"))).setText((currentPic + 1) + "/" + getImagesCount());
    }
    ...
    public void onClick(View view) {
        ...
        if(getChooseStateFromList(currentPic)){
            if (last_pics <= 0){
                toggleChooseState(currentPic);
                Toast.makeText(this, String.format(getString(fakeR.getId("string", "choose_pic_num_out_of_index")), total_pics), Toast.LENGTH_SHORT).show();
                return ;
            }
    ...
    private class MyViewPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            View view = LayoutInflater.from(PickBigImagesActivity.this).inflate(fakeR.getId("layout", "widget_zoom_iamge"), null);
            final ZoomImageView zoomImageView = (ZoomImageView) view.findViewById(fakeR.getId("id", "zoom_image_view"));
